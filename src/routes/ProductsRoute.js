...
router.get('/products/list', async (req, res) => {
    try {
        const { fromPrice, toPrice, category, sortKey = "createdAt", sortOrder = -1, page = 1 } = req.query;
        const limit = 9;
        const conditions = {
            isDeleted: "No",
        };
...
router.get('/products/listSameCategory', async (req, res) => {
    try {
        ...
        const result = await ProductsModel.find(conditions);
        if (result) {
            return res.json(responseSuccess("List product successfully!", result));
        }
        return res.json(responseSuccess("List product successfully!", []))
